-- SetupConfig: {}
--
-- @FacebookAccountId can not be null. This is the key that identifies a Facebook user.
--
...
--    If the facebook id is found, we update what we have to and output the found @UserId.
--
-- When @UserId is not 0, it must match with the one of the @FacebookAccountId otherwise it is an error
-- and an exception is thrown because:
--  - When updating it means that there is a mismatch of UserId/Facebook account in the calling code.
--  - When creating it means that another user with the same facebook account is already registered and
--    this should never happen.
--
-- When extending this procedure, during update null parameters must be left unchanged.
--
create procedure CK.sUserFacebookUCL
(
	@ActorId int,
	@UserId int /*input*/output,
	@FacebookAccountId varchar(36), 
...
begin
	...
	if @FacebookAccountId is null throw 50000, 'Argument.NullFacebookAccountId', 1;
	...
	select	@ActualUserId = UserId,
		...
		from CK.tUserFacebook 
		where FacebookAccountId = @FacebookAccountId;
	...
	begin
		...
		begin
			...
			-- Unique constraint on FacebookAccountId will detect any existing UserId/FacebookAccountId clashes.
	...
	begin
		...
		begin
			...
			else if @UserId <> @ActualUserId throw 50000, 'Argument.UserIdAndFacebookIdMismatch', 1;
			...
			update CK.tUserFacebook set 
					LastLoginTime = LastLoginTime 
				where UserId = @ActualUserId and FacebookAccountId = @FacebookAccountId;
	...
	begin
        ...
        begin
		    exec CK.sAuthUserOnLogin 'Facebook', @LastLoginTime, @UserId, @ActualLogin, @Now, @LoginFailureCode output, @LoginFailureReason output;  
            if @ActualLogin = 1 and @LoginFailureCode is null
            begin
			    update CK.tUserFacebook set LastLoginTime = @Now
                    where UserId = @UserId and FacebookAccountId = @FacebookAccountId;
            end
        end
	end
