...
{
    public abstract partial class UserFacebookTable
    {
        /// <summary>
        ...
        public UCLResult CreateOrUpdateFacebookUser( ISqlCallContext ctx, int actorId, int userId, IUserFacebookInfo info, UCLMode mode = UCLMode.CreateOrUpdate )
        {
            return UserFacebookUCL( ctx, actorId, userId, info, mode );
        }
        ...
        public LoginResult LoginUser( ISqlCallContext ctx, IUserFacebookInfo info, bool actualLogin = true )
        {
            var mode = actualLogin
                        ? UCLMode.UpdateOnly | UCLMode.WithActualLogin
                        : UCLMode.UpdateOnly | UCLMode.WithCheckLogin;
            var r = UserFacebookUCL( ctx, 1, 0, info, mode );
        ...
        public abstract void DestroyFacebookUser( ISqlCallContext ctx, int actorId, int userId );
        ...
        public IdentifiedUserInfo<IUserFacebookInfo> FindKnownUserInfo( ISqlCallContext ctx, string googleAccountId )
        {
        ...
        protected abstract UCLResult UserFacebookUCL(
            ISqlCallContext ctx,
            int actorId,
            int userId,
